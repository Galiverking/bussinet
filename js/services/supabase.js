...
import { SUPABASE_URL, SUPABASE_ANON_KEY, COLLECTION_JOBS, COLLECTION_EXPENSES, SYNC_STATUS } from '../core/constants.js';
...
export async function fetchJobs() {
  ...
  Store.set('jobs', data || []);
  console.log('[Supabase] Jobs loaded:', (data || []).length);
...
export async function insertJob(data) {
  ...
  const maxPri = Store.get('jobs').length > 0
...
export async function completeJob(id) {
  if (!supabase) return;
  return supabase.from(COLLECTION_JOBS).update({
    status: 'done',
...
export async function updateJobDistance(id, distanceKm) {
  if (!supabase) return;
  return supabase.from(COLLECTION_JOBS).update({ distance_km: distanceKm }).eq('id', id);
}
...
export function runAutoCleanup() {
  ...
  jobs.forEach(j => {
    if (j.status === 'done' && new Date(j.created_at).getTime() < cutoff) {
      supabase.from(COLLECTION_JOBS).delete().eq('id', j.id)
        .then(() => console.log('[AutoCleanup] Deleted job:', j.id))
  ...
  expenses.forEach(e => {
    if (new Date(e.created_at).getTime() < cutoff) {
      supabase.from(COLLECTION_EXPENSES).delete().eq('id', e.id)
        .then(() => console.log('[AutoCleanup] Deleted expense:', e.id))
        .catch(err => console.error('[AutoCleanup] Failed to delete expense:', e.id, err));
    }
  });

  console.log('[AutoCleanup] Complete');
}

// Helper to map job object to DB format
